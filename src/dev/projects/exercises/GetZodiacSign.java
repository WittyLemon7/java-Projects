package dev.projects.exercises;

public class GetZodiacSign {

    public static void main(String[] args) {
        System.out.println(getZodiacSign(3, 21)); // Example output: Aries
        System.out.println(getZodiacSign(4, 25)); // Example output: Taurus
        System.out.println(getZodiacSign(11, 15)); // Example output: Scorpio
    }

    public static String getZodiacSign(int month, int day) {
        if (month < 1 || month > 12) {
            return "Invalid month";
        }
        if (day < 1 || day > 31) {
            return "Invalid day";
        }

        return switch (month) {
            case 1 -> (day >= 20) ? "Aquarius" : "Capricorn";
            case 2 -> (day <= 18) ? "Aquarius" : "Pisces";
            case 3 -> (day <= 20) ? "Pisces" : "Aries";
            case 4 -> (day <= 19) ? "Aries" : "Taurus";
            case 5 -> (day <= 20) ? "Taurus" : "Gemini";
            case 6 -> (day <= 20) ? "Gemini" : "Cancer";
            case 7 -> (day <= 22) ? "Cancer" : "Leo";
            case 8 -> (day <= 22) ? "Leo" : "Virgo";
            case 9 -> (day <= 22) ? "Virgo" : "Libra";
            case 10 -> (day <= 22) ? "Libra" : "Scorpio";
            case 11 -> (day <= 21) ? "Scorpio" : "Sagittarius";
            case 12 -> (day <= 21) ? "Sagittarius" : "Capricorn";
            default -> "Invalid month";
        };
    }
}
